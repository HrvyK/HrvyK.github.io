...
const TIME_REG = /(?:(\d{2,4})[/-](\d{1,2})[/-](\d{1,2}))?\s*(?:(\d{1,2}):(\d{1,2})(?::(\d{1,2})(?::(\d{3}))?)?)?/u;
/**
...
export const parseDate = (date) => {
    ...
    if (isString(date)) {
        ...
        if (result.valueOf()) {
            const isDate = result.getHours() === 0 &&
                result.getMinutes() === 0 &&
                result.getSeconds() === 0 &&
            ...
            if (match) {
                ...
                if (isDef(year) && isDef(month) && isDef(day)) {
                    ...
                    return {
                        type: isDate ? 'date' : 'full',
                        value: isDate
                            ? new Date(year && Number(year) < 100 ? year + 2000 : year, month - 1, day)
                            : new Date(year && Number(year) < 100 ? year + 2000 : year, month - 1, day, hour, minute, hour && minute && !second ? 0 : second),
                    };
                }
            }
        }
...
export const dateSorter = (valueA, valueB) => {
    const dateA = parseDate(typeof valueA === 'number' ? new Date(valueA) : valueA)?.value;
    const dateB = parseDate(typeof valueB === 'number' ? new Date(valueB) : valueB)?.value;
    if (!dateA)
        return dateB ? 1 : 0;
    if (!dateB)
        return -1;
